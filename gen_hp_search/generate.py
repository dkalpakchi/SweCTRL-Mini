...
if __name__ == '__main__':
    ...
    grids = [
        {
            "temperature": np.arange(0, 1.1, 0.2),
            "repetition_penalty": np.arange(1, 2.1, 0.2)
        },
        {
            "top_p": np.arange(0.7, 1.02, 0.05),
            "repetition_penalty": np.arange(1, 2.1, 0.2)
        }
    ]
    grids[0]["temperature"][0] = 1e-15
    ...
    for grid in grids:
        combos = [list(zip(grid.keys(), d)) for d in cartesian_product(*grid.values())]
        print(len(combos))
        for combo in combos:
            print("Trying {}".format(combo))
            ...
            if top_p_data is None and temp_data is None:
                ...
                rep_p = combo_dict['repetition_penalty']
                if "temperature" in combo_dict:
                    t = combo_dict['temperature']
                    num_samples = args.num_samples - temp_data[rep_p][t]
                elif "top_p" in combo_dict:
                    top_p = combo_dict['top_p']
