...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model_size, num_heads, lora_r):
        super().__init__()
        ...
        self.Wq = lora.Linear(d_model_size, d_model_size, r=lora_r)
        self.Wk = nn.Linear(d_model_size, d_model_size)
        self.Wv = lora.Linear(d_model_size, d_model_size, r=lora_r)
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model_size, num_heads, dff, rate=0.1, lora_r=4):
        super().__init__()

        self.multi_head_attention = MultiHeadAttention(d_model_size, num_heads, lora_r=lora_r)
...
class CTRLModel(CTRLPreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList(
            [EncoderLayer(config.n_embd, config.n_head, config.dff, config.resid_pdrop, lora_r=config.lora_r) for _ in range(config.n_layer)]
        )
