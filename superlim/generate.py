...
from common import (
    ADDITIONAL_TOKENS_FILE, CONFIG_FILE, MODEL_FILE
)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lora', default="", type=str, help='Enable LoRA')
    ...
    tokenizer, START_C_CODES, END_C_CODES = load_tokenizer(
        ctrl_args.tokenizer_file,
        {"swefracas": ":swefracas:"},
        {"swefracas": ":swefracas:$"}
    )
 
    if args.lora:
        lora_parent = Path(args.lora).parent
        cfg_old_fname = os.path.join(args.file, CONFIG_FILE)
        cfg_fname = os.path.join(lora_parent, CONFIG_FILE)
        with open(cfg_fname) as f, open(cfg_old_fname) as fo:
            cfg_old = json.load(fo)
            cfg = json.load(f)
        cfg['vocab_size'] = cfg_old['vocab_size']
        model = ext_ctrl.CTRLLMHeadModel(
            config=transformers.PretrainedConfig.from_dict(cfg)
        )

        model.load_state_dict(
            torch.load(os.path.join(args.file, MODEL_FILE)),
            strict=False
        )

        Nt = len(tokenizer)

        if (Nt > model.config.vocab_size):
            weights = model.transformer.w.weight.detach().numpy()
            vecs = torch.load(os.path.join(lora_parent, ADDITIONAL_TOKENS_FILE))
            new_weights = np.vstack((
                weights,
                np.array([v for _, v in sorted(vecs.items(), key=itemgetter(0))])
            ))
            reassign_embeddings(model, new_weights)
            model.config.vocab_size = new_weights.shape[0]
            model.vocab_size = model.config.vocab_size
            model.tie_weights()

        # Then load the LoRA checkpoint
        model.load_state_dict(
            torch.load(os.path.join(args.lora, MODEL_FILE)),
            strict=False
        )
    else:
    ...
    eos_token_id = tokenizer.convert_tokens_to_ids(END_C_CODES["swefracas"])
    #if args.control:
